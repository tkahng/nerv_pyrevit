...
for i, j, y in os.walk(path):
    if i[-5:] == 'panel':
        # sprint(j)
        dir = os.path.join(docPath, os.path.basename(i[:-6]))
        # print(dir)
        ...
        if not os.path.isfile(dir + "\\index.rst"):
            ...
            f.write(os.path.basename(i[:-6]) + index + 'nerv-' + os.path.basename(i[:-6]).lower())
            f.close()

        else:
            # print(os.path.basename(i[:-6]) + " panel index already exists")
            pass

        # Make the directory of a panel in mater index file
        pathText = "nerv/{0}/index.rst".format(os.path.basename(i[:-6]))
        ...
        for item in j:
            if ".pushbutton" in item:
                ...
                imageDis = dir + "\\" + "_static" + "\\" + item[:-11].lower() + ".png"
                shutil.copy(i + '\\' + item + "\\" + "icon.png", imageDis)
                pushButtonFile = open(dir + "\\" + item[:-11].lower() + ".rst", "w")
                pushButtonFile.write(item[:-11] + buttonTemplate.format("_static" + "/" + item[:-11].lower() + ".png") + item[:-11].lower())
                pushButtonFile.close()
                panelIndex = open(dir + "\\index.rst", "a+")
                panelIndexRead = open(dir + "\\index.rst", "r").read()
                if not item[:-11].lower() in panelIndexRead:
                    panelIndex.write("\n    " + item[:-11].lower())
            else:
                subName = item.split(".")[0]
