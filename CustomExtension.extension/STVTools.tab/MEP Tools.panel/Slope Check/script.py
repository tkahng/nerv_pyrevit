...
def SetElbow(pipeType,ElementId):
    pipeType.get_Parameter(BuiltInParameter.RBS_CURVETYPE_DEFAULT_ELBOW_PARAM).Set(ElementId)


t = Transaction(doc, 'Switching Elbow')
t.Start()
if not CheckSelection(doc) is None:
    type = CheckSelection(doc)
else:
    type = PipeSelect(doc)
fitting = FindPipeFittings(doc, 'Elbow')
SetElbow(type, fitting)
t.Commit()
