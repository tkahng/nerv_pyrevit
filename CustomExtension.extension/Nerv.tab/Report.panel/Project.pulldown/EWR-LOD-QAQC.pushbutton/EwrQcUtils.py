...
def FamilyNameCheck(doc):
    ...
    exempt = ['Profiles', 'Section Marks', 'Curtain Panels', 'Section Marks', 'Generic Annotations', 'Callout Heads',
              'Level Heads', 'View Titles','Spot Elevation Symbols', 'Grid Heads', 'Detail Items', 'Title Blocks', 'Elevation Marks']
    for i in family:
        line = []
...
def ElementinWorksetCheck(doc):
    ...
    worksetElement =[]
    for i in allModel:
        workset = i.get_Parameter(BuiltInParameter.ELEM_PARTITION_PARAM).AsValueString()
        if '350' in workset or 'ME - ' in workset or 'Fire alarm Devices' in workset:
            worksetElement.append(i)
    allLst = []
    raw = []
    finalLst = []
    family = 'None'
    for instance in worksetElement:
        cate = instance.Category
        all_param = get_all_parameters(instance)
        for i in all_param:

            if i[0:9] == 'Type Name' and not 'None' in i:
                family = i
            elif i[0:5] == 'Type' and not 'None' in i:
                family = i
            else:
                family = instance.get_Parameter(BuiltInParameter.ELEM_FAMILY_AND_TYPE_PARAM).AsValueString()
        ...
        if str(cate) != 'None':
            ...
            if instance.GetType() != AnnotationSymbol and str(cate.CategoryType) == 'Model' and \
                    ...
                    and a != 'Materials' and a != 'RVT Links' and a != 'Areas' and a != 'Project Information':
                workset = instance.get_Parameter(BuiltInParameter.ELEM_PARTITION_PARAM).AsValueString()
                combine = str(cate.Name) + ' : ' + str(family) + ' : ' + str(workset)
                raw.append(combine)
                if not combine in allLst:
                    allLst.append(combine)
    for item in allLst:
        length = raw.count(item)
        finalLst.append(item + ' : ' + str(length))
    for i in finalLst:
        a = re.split(' : ', i)
        modelLst.append(a)
    return modelLst